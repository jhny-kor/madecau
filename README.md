...
이미 복호화된 **일반 파일**은 첨부 시점에야 알 수 있습니다. `일반파일은 첨부할 수 없습니다` 팝업이 파일마다 하나씩 뜨는데, 전부 확인을 눌러 닫고 **그 파일만 건너뛴 채 계속** 진행합니다. 건너뛴 파일은 `건너뜀 (첨부 거부됨)` 으로 남고 마지막 줄에 집계됩니다. 팝업이 없으면 곧바로 제목 입력으로 넘어갑니다.
