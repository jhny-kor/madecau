...
`백그라운드 실행` 을 켜면(기본값) 실제 마우스를 움직이지 않고 창에 클릭 메시지를 보냅니다. 자동화가 도는 동안 다른 작업을 할 수 있고, MarkAny 창이 가려져 있어도 동작합니다.

오너 드로우로 만들어진 컨트롤은 클릭 메시지에 반응하지 않을 수 있습니다. 특히 `사전/사후 콤보` 와 파일 대화상자에서 멈추면 이 체크를 끄고 실제 마우스로 실행하세요.

### 중지

**Ctrl+Shift+Q** 를 누르면 중지합니다. GUI의 `중지` 버튼도 같습니다. 진행 중인 단계가 끝나는 시점에 멈춥니다.

백그라운드가 꺼져 있을 때는 **ESC** 로도 중지됩니다. 백그라운드 실행 중에는 다른 작업을 하다 ESC 를 누를 수 있어 받지 않습니다.
